...
**分析管线**: .NET Framework 4.8 + SeeSharpTools DSP + Python (scipy/pandas/matplotlib)
...
**注意**: 上述 p 值基于全部 6,432 个 epoch 直接 pool 计算，未考虑被试内嵌套结构，因此 p 值被严重低估（伪重复）。当前版本尚未实现线性混合效应模型 (LME)，上述显著性结果仅作效应方向参考。后续将采用以被试为随机截距的 LME 重新分析，以更严格地检验 Q1 效应。
...
5. **统计模型**: 当前分析采用被试内平均 + 配对 t 检验，尚未实现被试随机效应的线性混合效应模型（LME）；后续将补充该分析以进一步验证结论。
